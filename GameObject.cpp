...
GameObject::GameObject(ObjModel* model,float x, float y, float z)
{
	pModel = model;
	pX = x;
	pY = y;
...
void GameObject::DrawObject(ObjModel* model,float rotation, glm::vec3 scale) {
	glm::mat4 modelMatrix = glm::mat4(1.0f);
	modelMatrix = TranslateObject(modelMatrix, pX, pY, pZ);
