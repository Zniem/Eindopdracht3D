...
void ObjModel::draw()
{
	...
	{
		ObjModel::MaterialInfo* material = nullptr;
		if (group->materialIndex >= 0 && group->materialIndex < materials.size())
		...
		if (material)
		{
			...
			tigl::shader->setColorMult(glm::vec4(material->diffuseColor, 1.0f));
		}
		else
		{
		...
		for (const auto& face : group->faces)
		{
			for (const auto& v : face.vertices)
			{
				...
				vert.position = vertices[v.position];
				vert.normal = (v.normal >= 0 && v.normal < normals.size()) ? normals[v.normal] : glm::vec3(0, 1, 0);
				vert.texcoord = (v.texcoord >= 0 && v.texcoord < texcoords.size()) ? texcoords[v.texcoord] : glm::vec2(0, 0);
				vert.color = glm::vec4(1, 1, 1, 1); // default white; colorMult is applied in shader
...
void ObjModel::loadMaterialFile( const std::string &fileName, const std::string &dirName )
{
	...
	while(!pFile.eof())
	{
		...
		{
			currentMaterial->diffuseColor = glm::vec3(
				std::stof(params[1]),
				std::stof(params[2]),
				std::stof(params[3])
			);
